...
function extractPostConditional(inputObject) {
    ...
    for (var i = 0; i < tmpObject.length; i++) {
        if (tmpObject[i].mRoutine) {
            if (tmpObject[i].mRoutine.match(":")) {
                var tmpSplitObject = tmpObject[i].mRoutine.split(":");
                tmpObject[i].mRoutine = tmpSplitObject[0];
                tmpObject[i].mPostConditional = tmpSplitObject[1];
            }
        }
    }
